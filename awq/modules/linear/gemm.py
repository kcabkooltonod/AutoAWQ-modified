...
class WQLinear_GEMM(nn.Module):
    ...
    #         linear.weight.device,
    #     )
    #     if init_only:  # 仅用于加载模型，不进行量化
    #         return awq_linear

    #     # 需要 scales 和 zeros 信息进行量化
    #     assert scales is not None and zeros is not None
    #     scale_zeros = zeros * scales
    ...
    #     # for idx in range(awq_linear.in_features):
    #     #     # 计算当前特征索引对应的二维块索引
    #     #     block_row = idx // (group_size_2d * group_size_2d)
    #     #     block_col = (idx % (group_size_2d * group_size_2d)) // group_size_2d
    #     #     block_idx = block_row * group_size_2d + block_col

    #     #     # 对权重进行量化
    #     #     intweight.append(
    #     #         torch.round(
    #     #             (linear.weight.data[:, idx] + scale_zeros[block_idx])
    #     #             / awq_linear.scales[block_idx]
    #     #         ).to(torch.int)[:, None]
    #     #     )
    #     # intweight = torch.cat(intweight, dim=1)
    #     intweight = torch.round((linear.weight.data + scale_zeros) / awq_linear.scales).to(torch.int)
    #     intweight = intweight.t().contiguous()
    #     intweight = intweight.to(dtype=torch.int32)

    #     # 选择最佳设备（避免 MPS 设备不支持某些操作）
    #     best_device = get_best_device()
    #     if "mps" in best_device:
    #         intweight = intweight.to("cpu")
